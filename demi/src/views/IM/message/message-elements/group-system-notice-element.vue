  ...
  methods: {
    translateGroupSystemNotice(payload) {
      ...
      switch (payload.operationType) {
        case 1:
          return `${this.operator} 申请加入群组：${groupName}`
        case 2:
          return `成功加入群组：${groupName}`
        case 3:
          return `申请加入群组：${groupName}被拒绝`
        case 4:
          return `被管理员${this.operator}踢出群组：${groupName}`
        case 5:
          return `群：${groupName} 已被${this.operator}解散`
        case 6:
          return `${this.operator}创建群：${groupName}`
        case 7:
          return `${this.operator}邀请你加群：${groupName}`
        case 8:
          return `你退出群组：${groupName}`
        case 9:
          return `你被${this.operator}设置为群：${groupName}的管理员`
        case 10:
          return `你被${this.operator}撤销群：${groupName}的管理员身份`
        case 255:
          return '自定义群系统通知'
      }
    },
