...
const l = 42 // 滑块边长
const r = 10 // 滑块半径
const w = 282 // canvas宽度
const h = 120 // canvas高度
const PI = Math.PI
...
export default class Jigsaw {
  ...
  initDOM() {
    ...
    const canvas_wrap = createElement('div')
    ...
    canvas_wrap.appendChild(refreshIcon)
    canvas_wrap.appendChild(block)
    el.appendChild(canvas_wrap)
    slider.appendChild(sliderIcon)
    sliderMask.appendChild(slider)
    sliderContainer.appendChild(sliderMask)
    sliderContainer.appendChild(text)
    canvas_wrap.appendChild(sliderContainer)
  ...
  clean() {
    ...
    this.block.width = w
    console.log(this.canvas_wrap)
    console.log(this.el)
    this.el.removeChild(this.canvas_wrap)
  }
  ...
  over(){
    if(this.el.getElementsByTagName('div').length === 0){
